...
func TestBuildEngine(t *testing.T) {
	...
	engine, cleanup, err := buildEngine(ctx, dbPath, stoplistPath, dictPath, "", taxonomyPath, rulesPath, true)
	if err != nil {
...
func TestBuildEngineNonExistentStoplist(t *testing.T) {
	...
	_, _, err := buildEngine(ctx, dbPath, stoplistPath, dictPath, "", taxonomyPath, "", true)
	if err == nil {
...
func TestBuildEngineMalformedRules(t *testing.T) {
	...
	_, _, err := buildEngine(ctx, dbPath, stoplistPath, dictPath, "", taxonomyPath, rulesPath, true)
	if err == nil {
...
func TestExecuteQueryEmptyDB(t *testing.T) {
	...
	engine, cleanup, err := buildEngine(ctx, dbPath, stoplistPath, dictPath, "", taxonomyPath, "", true)
	if err != nil {
		t.Fatalf("buildEngine failed: %v", err)
	}
	defer cleanup()
