...
func runIterativeAnalysis(ctx context.Context, items []rss.Item, cfg iterationConfig) iterationResult {
	...
	for i := 0; i < cfg.maxIterations; i++ {
		...
		for _, item := range items {
			normalizedText := cleanBootstrapText(item.Body)
...
func runBootstrap() {
	...
	highDF := analytics.TopHighDF(stats, 20)
...
func generateStopwords(ctx context.Context, stats analytics.Stats, baseStoplistPath string, noBaseStoplist bool, limit int, llmBase, llmModel, llmKey string) []string {
	...
	if llmBase != "" && llmModel != "" {
		rev := &reviewer.Client{Endpoint: llmBase, APIKey: llmKey}
		candidates = rev.ReviewStopwords(ctx, candidates, limit)
...
func writeReport(path string, stats analytics.Stats, stopwords []string, pairs []analytics.PairStat, synonyms []synonymCandidate, highDF []analytics.HighDFEntry, taxonomy map[string][]string, iterationLog []iterationStep) error {
	report := struct {
		...
		HighDF       []analytics.HighDFEntry `json:"high_df_tokens"`
...
func cleanBootstrapText(text string) string {
