...
// TestCategorizeHN tests keyword-based categorization
func TestCategorizeHN(t *testing.T) {
	tests := []struct {
		name  string
		title string
		url   string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := categorizeHN(tt.title, tt.url)
			if !equalStringSlices(got, tt.want) {
				t.Errorf("categorizeHN(%q, %q) = %v, want %v", tt.title, tt.url, got, tt.want)
			}
		})
	}
}
